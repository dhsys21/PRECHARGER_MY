...
//                }
                switch(currentWriteTask)
                {
                    case nPCDATA:
                        ...
                        //* voltage, current write 완료
                        if(GetPlcValue(PLC_D_PRE_COMPLETE) == 1)
                            SetValue(PC_D_PRE_COMPLETE, 0);
			...
			if(plc_Read.Length() >= length)
			{
                switch(currentReadTask)
                {
                    case nSTANDARD:
                        ...
                        if(GetPlcValue(PLC_D_PRE_CELL_SERIAL_START) == 1 && GetValue(PC_D_PRE_CELL_SERIAL_COMP) == 0)
...
//---------------------------------------------------------------------------
AnsiString __fastcall TMod_PLC::GetPlcString(int plc_address, int size)
{
