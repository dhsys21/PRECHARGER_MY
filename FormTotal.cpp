...
void __fastcall TTotalForm::PLCInitialization()
{
    ...
    Mod_PLC->SetDouble(Mod_PLC->pc_Interface_Data, PC_D_PRE_COMPLETE, 0);
    Mod_PLC->SetValue(PC_D_PRE_REMEASURE, 0);

    Mod_PLC->SetValue(PC_D_PRE_CELL_SERIAL_COMP, 0);
    Mod_PLC->SetValue(PC_D_PRE_CELL_SERIAL_START, 0);
    Mod_PLC->SetValue(PC_D_PRE_CELLID_BYPASS, 0);
...
void __fastcall TTotalForm::Timer_AutoInspectionTimer(TObject *Sender)
{
	if(stage.arl == nAuto && Mod_PLC->GetDouble(Mod_PLC->pc_Interface_Data, PC_D_PRE_STAGE_AUTO_READY) == 0){
        ...
        Mod_PLC->SetValue(PC_D_PRE_STAGE_AUTO_READY, 1);
        ...
        Mod_PLC->SetValue(PC_D_PRE_STAGE_AUTO_READY, 0);
		PreChargerStatus = "PreCharger STAGE AUTO READY = 0";
		WritePLCLog("PreCharger STAGE AUTO/MANUAL", PreChargerStatus);
	}

    if(chkCellIdBypass->Checked == true){
        Mod_PLC->SetValue(PC_D_PRE_CELLID_BYPASS, 1);
    } else{
        Mod_PLC->SetValue(PC_D_PRE_CELLID_BYPASS, 0);
    }
...
bool __fastcall TTotalForm::ErrorCheck()
{
    ...
    if(charge[0].volt == 0 && charge[0].curr == 0){
        Panel_State->Caption = "No Setting Values.";
        return true;
    }

    if(Mod_PLC->GetPlcValue(PLC_D_PRE_TRAY_IN) == 0
...
void __fastcall TTotalForm::AutoInspection_Wait()
{
	...
	switch(nStep)
	{
		...
		case 1:
            ...
            trayid = Mod_PLC->GetPlcString(PLC_D_PRE_TRAY_ID, 10);
		...
		case 3:
			if(tray.cell_count > 0)
			{
                ...
                Mod_PLC->SetValue(PC_D_PRE_PROB_CLOSE, 1);
...
void __fastcall TTotalForm::AutoInspection_Measure()
{
	...
	if(tray.ams == true && tray.amf == false){
        Mod_PLC->SetValue(PC_D_PRE_CHARGING, 1);
		DisplayProcess(sCharge, "AutoInspection_Measure", " Start charging ... ");
	}
	else{
        Mod_PLC->SetValue(PC_D_PRE_CHARGING, 0);
	}
	...
	switch(nStep)
	{
		case 0:
			...
			if(plc_probe_close == 1 && plc_tray_in == 1)
			{
                ...
                Mod_PLC->SetValue(PC_D_PRE_PROB_CLOSE, 0);
                Mod_PLC->SetValue(PC_D_PRE_REMEASURE, 0);
		...
		case 4:
			...
			if(plc_probe_open == 1 && tray.amf == true)
			{
                ...
                Mod_PLC->SetValue(PC_D_PRE_PROB_OPEN, 0);
	...
	switch(nStep)
	{
		case 0:
			...
			if(plc_tray_in == 0)
			{
                ...
                Mod_PLC->SetValue(PC_D_PRE_TRAY_OUT, 0);
                Mod_PLC->SetValue(PC_D_PRE_PROB_OPEN, 0);
                Mod_PLC->SetValue(PC_D_PRE_PROB_CLOSE, 0);
...
void __fastcall TTotalForm::ChannelStatus()
{
    ...
    int extraTime = 19; //* 2026 05 22 step을 1단계 precharge2로만 했을 경우 종료를 빠르게 해야함.
...
void __fastcall TTotalForm::WriteVoltCurrValue()
{
	...
	for(int i = 0; i < MAXCHANNEL; i++)
	{
        //int32_t voltage_int = static_cast<int32_t>(BaseForm->StringToDouble(real_data.final_volt[i], 0) * 10.0);
	...
	for(int i = 0; i < MAXCHANNEL; i++)
	{
//        int32_t current_int = static_cast<int32_t>(BaseForm->StringToDouble(real_data.final_curr[i], 0) * 10.0);
...
void __fastcall TTotalForm::WriteMINMAX(int stage_num)
{
	...
	//* 2021 02 05
    Mod_PLC->SetSpecValue(PC_D_PRE_CURRENT_MIN, currMin * 10);
    Mod_PLC->SetSpecValue(PC_D_PRE_CHARGE_VOLTAGE, setChargeVolt);
    Mod_PLC->SetSpecValue(PC_D_PRE_CHARGE_CURRENT, setChargeCurr);
    Mod_PLC->SetSpecValue(PC_D_PRE_CHARGE_TIME, setChargeTime);
//	Mod_PLC->SetDouble(Mod_PLC->pc_Interface_Data, PC_D_PRE_CURRENT_MIN, currMin * 10);
...
void __fastcall TTotalForm::btnTrayOutClick(TObject *Sender)
{
	if(MessageBox(Handle, L"Are you sure you want to eject the tray?", L"", MB_YESNO|MB_ICONQUESTION) == ID_YES){
        Mod_PLC->SetValue(PC_D_PRE_PROB_CLOSE, 0);
        Mod_PLC->SetValue(PC_D_PRE_PROB_OPEN, 1);
        Mod_PLC->SetValue(PC_D_PRE_COMPLETE, 1);
        Mod_PLC->SetValue(PC_D_PRE_TRAY_OUT, 1);
...
void __fastcall TTotalForm::ReadCellSerial2()
{
    ...
    switch(nCellSerialStep){
        case 0:
            ...
            else if(Mod_PLC->GetPlcValue(PLC_D_PRE_CELL_SERIAL_START) == 1){
                nCellSerialStep = 1;
        	...
        	break;
        case 1:
            Mod_PLC->SetValue(PC_D_PRE_CELL_SERIAL_START, 1);
            nCellSerialCount = Mod_PLC->GetPlcValue(PLC_D_PRE_CELL_SERIAL_COUNT);
            // trayID = Mod_PLC->GetCellSrialTrayId(PLC_D_PRE_CELL_SERIAL_TRAYID, 10);

            for(int i = 0; i < MAXCHANNEL; i++){
                m_sCell_Serial[i] = Mod_PLC->GetCellSrial(PLC_D_PRE_CELL_SERIAL, i, 10);
            ...
            if(trayID.IsEmpty() == false && (nCellSerialCount == nCellSerial
                ...
                Mod_PLC->SetValue(PC_D_PRE_CELL_SERIAL_COMP, 1);
                SaveTrayInfo(trayID);
                nCellSerialStep = 2;
            ...
            break;
        case 2:
            if(Mod_PLC->GetPlcValue(PLC_D_PRE_CELL_SERIAL_COMP) == 1){
                Mod_PLC->SetValue(PC_D_PRE_CELL_SERIAL_COMP, 0);
                Mod_PLC->SetValue(PC_D_PRE_CELL_SERIAL_START, 0);
            ...
            if(Mod_PLC->GetValue(PC_D_PRE_CELL_SERIAL_START) == 0
                && Mod_PLC->GetValue(PC_D_PRE_CELL_SERIAL_COMP) == 0){
