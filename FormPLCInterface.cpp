...
void __fastcall TForm_PLCInterface::SetListViewPLC(int nTag)
{
    ...
    AddListView(nListView_PLC[nTag], "D" + IntToStr(PLC_D_INTERFACE_START_DEV_NUM + PLC_D_PRE_COMPLETE), "READ COMPLETE");
	AddListView(nListView_PLC[nTag], "D" + IntToStr(PLC_D_INTERFACE_START_DEV_NUM + PLC_D_PRE_TRAY_ID), "TRAY ID");

    //* Cell Serial Data
    AddListView(nListView_PLC[nTag], "D" + IntToStr(PLC_D_INTERFACE_START_DEV_NUM + PLC_D_PRE_CELL_SERIAL_START), "CELL SERIAL WRITE START");
	AddListView(nListView_PLC[nTag], "D" + IntToStr(PLC_D_INTERFACE_START_DEV_NUM + PLC_D_PRE_CELL_SERIAL_COMP), "CELL SERIAL WRITE COMPLETE");
	AddListView(nListView_PLC[nTag], "D" + IntToStr(PLC_D_INTERFACE_START_DEV_NUM + PLC_D_PRE_CELL_SERIAL_COUNT), "CELL SERIAL COUNT");
    ...
    //* CELL SERIAL DATA
    AddListView(nListView_PLC[nTag], "D" + IntToStr(PLC_D_CELL_SERIAL_NUM + PLC_D_PRE_CELL_SERIAL_TRAYID), "CELL SERIAL TRAY ID");
...
void __fastcall TForm_PLCInterface::SetListViewPC(int nTag)
{
    ...
    AddListView(nListView_PC[nTag], "D" + IntToStr(PC_D_INTERFACE_START_DEV_NUM + PC_D_PRE_COMPLETE), "WRITE COMPLETE");
    AddListView(nListView_PC[nTag], "D" + IntToStr(PC_D_INTERFACE_START_DEV_NUM + PC_D_PRE_REMEASURE), "REMEASURE");
    ...
    // CELL SERIAL
    AddListView(nListView_PC[nTag], "D" + IntToStr(PC_D_INTERFACE_START_DEV_NUM + PC_D_PRE_CELL_SERIAL_START), "CELL SERIAL READ START");
    AddListView(nListView_PC[nTag], "D" + IntToStr(PC_D_INTERFACE_START_DEV_NUM + PC_D_PRE_CELL_SERIAL_COMP), "CELL SERIAL READ COMPLETE");
    AddListView(nListView_PC[nTag], "D" + IntToStr(PC_D_INTERFACE_START_DEV_NUM + PC_D_PRE_CELLID_BYPASS), "CELL ID BYPASS");
...
//		AddListView(nListView_PC[nTag], "D" + IntToStr(PC_D_INTERFACE_CURRENT + PC_D_PRE_CURRENT_VALUE + i * 2), "PRECHARGE CURRENT #" + IntToStr(i + 1));
    for(int i = 0; i < 20; i++)
		AddListView(nListView_PC[nTag], "D" + IntToStr(PC_D_INTERFACE_VOLTAGE + PC_D_PRE_VOLTAGE_VALUE + (i * 2 * 20)), "PRECHARGE VOLTAGE #" + IntToStr(i * 20 + 1));

	for(int i = 0; i < 20; i++)
		AddListView(nListView_PC[nTag], "D" + IntToStr(PC_D_INTERFACE_CURRENT + PC_D_PRE_CURRENT_VALUE + (i * 2 * 20)), "PRECHARGE CURRENT #" + IntToStr(i * 20 + 1));
}//---------------------------------------------------------------------------
...
void __fastcall TForm_PLCInterface::Timer_UpdateTimer(TObject *Sender)
{
	...
	if(Mod_PLC->ClientSocket_PLC->Active)	//	PLC 상태
	{
		for(int nTag = 0; nTag < BaseForm->FormCnt; nTag++){
            ...
            nListView_PLC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetDouble(Mod_PLC->plc_Interface_Data, PLC_D_PRE_COMPLETE);
			nListView_PLC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetString(Mod_PLC->plc_Interface_Data, PLC_D_PRE_TRAY_ID, 10);

            //* CELL SERIAL
            nListView_PLC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetPlcValue(PLC_D_PRE_CELL_SERIAL_START);
			nListView_PLC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetPlcValue(PLC_D_PRE_CELL_SERIAL_COMP);
            nListView_PLC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetPlcValue(PLC_D_PRE_CELL_SERIAL_COUNT);
			...
			//* CELL SERIAL
			int serialIndex = 0;
            nListView_PLC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetCellSrialTrayId(PLC_D_PRE_CELL_SERIAL_TRAYID, 10);
			for(int i = 0; i < 20; i++){
				serialIndex = i * 20;
				nListView_PLC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetCellSrial(PLC_D_PRE_CELL_SERIAL, serialIndex, 10);
			}
		}
	}
	...
	if(Mod_PLC->ClientSocket_PC->Active)	//	PC 상태
	{
		for(int nTag =0; nTag < BaseForm->FormCnt; nTag++)
		{
            ...
            nListView_PC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetDouble(Mod_PLC->pc_Interface_Data, PC_D_PRE_COMPLETE);
            nListView_PC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetDouble(Mod_PLC->pc_Interface_Data, PC_D_PRE_REMEASURE);
            ...
            //* CELL SERIAL
			nListView_PC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetDouble(Mod_PLC->pc_Interface_Data, PC_D_PRE_CELL_SERIAL_START);
			nListView_PC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetDouble(Mod_PLC->pc_Interface_Data, PC_D_PRE_CELL_SERIAL_COMP);
            nListView_PC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetDouble(Mod_PLC->pc_Interface_Data, PC_D_PRE_CELLID_BYPASS);
...
//			for(int i = 0; i < MAXCHANNEL; i++)
//			{
//				nListView_PC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetVoltValue(PC_D_PRE_VOLTAGE_VALUE, i);
//			}
            for(int i = 0; i < 20; i++)
                nListView_PC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetVoltValue(PC_D_PRE_VOLTAGE_VALUE, i * 20);

			// PRECHARGE RESULT CURRENT VALUE
//			for(int i = 0; i < MAXCHANNEL; i++)
//			{
//				nListView_PC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetCurrValue(PC_D_PRE_CURRENT_VALUE, i);
//			}
            for(int i = 0; i < 20; i++)
                nListView_PC[nTag]->Items->Item[index++]->SubItems->Strings[1] = Mod_PLC->GetCurrValue(PC_D_PRE_CURRENT_VALUE, i * 20);
		}
	}
}
//---------------------------------------------------------------------------
...
{
    int address = cbAddress->Text.ToIntDef(42000);
	int value = editValue->Text.ToIntDef(1);
	Mod_PLC->SetDouble(Mod_PLC->pc_Interface_Curr_Data, address - 42000, value);
}
//---------------------------------------------------------------------------
